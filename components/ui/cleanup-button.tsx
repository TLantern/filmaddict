  ...
  return (
    <StyledWrapper $active={active}>
      <button className="button" type={type} disabled={disabled || loading}>
        ...
        {loading ? (
          <>
            <Loader2 size={28} className="sparkle-loader" />
            <span className="text_button">Processing...</span>
          </>
        ) : (
          <>
            ...
            <span className="text_button">Clean up</span>
          </>
        )}
      </button>
    </StyledWrapper>
...
const StyledWrapper = styled.div<{ $active?: boolean }>`
  ...
  .button .sparkle .path {
    ...
    color: hsl(0, 0%, 100%);
    animation: ${props => props.$active ? 'path 1.5s linear 0.5s infinite' : 'none'};
  }

  .button:is(:hover, :focus):not(:disabled) .sparkle .path {
